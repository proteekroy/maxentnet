...
from temp import torchvision_resnet as torchvision_resnet
...
class MNISTDataLoader(PrivacyDataLoader):
    def __init__(self, train_data=None, train_label=None, train_sensitive_label=None, test_data=None, test_label=None,
                 test_sensitive_label=None, trainset=None, testset=None):
        self.name = "mnist"
        super().__init__(train_data, train_label, train_sensitive_label, test_data, test_label, test_sensitive_label, trainset, testset)
    ...
    def load(self):
        ...
        self.circle_set = np.array([0, 6, 8, 9])
        self.bar_set = np.array([1, 4, 7])
        self.curve_set = np.array([2, 3, 5])

        trainset = torchvision.datasets.MNIST('../data/mnist', download=True, train=True, transform=transform_train)
        testset = torchvision.datasets.MNIST('../data/mnist', download=True, train=False, transform=transform_test)

        self.train_data = trainset.train_data
        self.train_sensitive_label = trainset.train_labels
        self.test_data = testset.test_data
        self.test_sensitive_label = testset.test_labels
        self.train_size = self.train_data.shape[0]
        self.test_size = self.test_data.shape[0]

        target = np.zeros(self.train_size)
        circle = np.in1d(self.train_sensitive_label.numpy(), self.circle_set)  # .astype('float')  # construct target label
        bar = np.in1d(self.train_sensitive_label.numpy(), self.bar_set)
        curve = np.in1d(self.train_sensitive_label.numpy(), self.curve_set)
        ...
        target = np.zeros(self.test_size)
        circle = np.in1d(self.test_sensitive_label.numpy(), self.circle_set)  # .astype('float')  # construct target label
        bar = np.in1d(self.test_sensitive_label.numpy(), self.bar_set)
        curve = np.in1d(self.test_sensitive_label.numpy(), self.curve_set)

        target[circle] = 0
        target[bar] = 1
        target[curve] = 2

        self.test_label = torch.from_numpy(copy.deepcopy(target)).long()

        self.train_data = torch.unsqueeze(self.train_data, 1).float()
        self.test_data = torch.unsqueeze(self.test_data, 1).float()

        self.trainset = MyDataset(self.train_data, self.train_label, self.train_sensitive_label)
        self.testset = MyDataset(self.test_data, self.test_label, self.test_sensitive_label)


        self.net = MNISTNet(self.n_target_class, self.n_sensitive_class)
        self.discriminator_net = create_discriminator(embed_length=self.embed_length, num_classes=self.n_sensitive_class, type=1)
        self.adversary_net = create_discriminator(embed_length=self.embed_length, num_classes=self.n_sensitive_class, type=1)
        self.optimizer = optim.Adam(self.net.parameters(), lr=0.0001, weight_decay=5e-3,
                                    betas=(0.9, 0.999))
        self.discriminator_optimizer = optim.Adam(self.discriminator_net.parameters(),
                                                  lr=0.01, weight_decay=1e-3, betas=(0.9, 0.999))
        self.adv_optimizer = optim.Adam(self.adversary_net.parameters(),
...
class SwissRollDataLoader(PrivacyDataLoader):
    def __init__(self, train_data=None, train_label=None, train_sensitive_label=None, test_data=None, test_label=None,
                 test_sensitive_label=None, trainset=None, testset=None):
    ...
    def load(self):
        # n = 4000
        # data, color = skdatasets.samples_generator.make_swiss_roll(n_samples=n)
        # index = np.argsort(color)
        #
        # label = np.zeros(n)
        # label[index[0:1000]] = 0
        # label[index[1000:2000]] = 1
        # label[index[2000:3000]] = 2
        # label[index[3000:n]] = 3
        # self.label = label
        # self.all_data = data
        #
        # target_label = (label == 2) | (label == 3)
        # target_label = target_label.astype(int)
        #
        # sensitive_label = (label == 1) | (label == 3)
        # sensitive_label = sensitive_label.astype(int)
        # # sensitive_label = label
        #
        # a = np.arange(n)
        # np.random.shuffle(a)
        # data = data[a]
        # sensitive_label = sensitive_label[a]
        # target_label = target_label[a]
        #
        # train_data = data[0:3000, :]
        # train_label = target_label[0:3000]
        # train_sensitive_label = sensitive_label[0:3000]
        #
        # test_data = data[3000:4000, :]
        # test_label = target_label[3000:4000]
        # test_sensitive_label = sensitive_label[3000:4000]
        #
        # np.savetxt('../data/swissroll/train_data_swiss_roll.txt', train_data)
        # np.savetxt('../data/swissroll/train_label_swiss_roll.txt', train_label)
        # np.savetxt('../data/swissroll/train_sensitive_label_swiss_roll.txt', train_sensitive_label)
        # np.savetxt('../data/swissroll/test_data_swiss_roll.txt', test_data)
        # np.savetxt('../data/swissroll/test_label_swiss_roll.txt', test_label)
        # np.savetxt('../data/swissroll/test_sensitive_label_swiss_roll.txt', test_sensitive_label)
        # np.savetxt('../data/swissroll/label_swiss_roll.txt', label)

        train_data = np.loadtxt('../data/swissroll/train_data_swiss_roll.txt')
        train_label = np.loadtxt('../data/swissroll/train_label_swiss_roll.txt').astype(np.long)
        train_sensitive_label = np.loadtxt('../data/swissroll/train_sensitive_label_swiss_roll.txt').astype(np.long)
        test_data = np.loadtxt('../data/swissroll/test_data_swiss_roll.txt')
        test_label = np.loadtxt('../data/swissroll/test_label_swiss_roll.txt').astype(np.long)
        test_sensitive_label = np.loadtxt('../data/swissroll/test_sensitive_label_swiss_roll.txt').astype(np.long)

        self.label = np.loadtxt('../data/swissroll/label_swiss_roll.txt')
        self.all_data = np.concatenate((train_data, test_data), axis=0)
        self.n_target_class = 2
        ...
        self.net = SwissRollNet(num_classes=self.n_target_class, embed_length=self.embed_length)
...
class GaussianDataLoader(PrivacyDataLoader):
    ...
    def load(self):
        ...
        raw_input = np.loadtxt('../data/toy/toy_problem_input.txt')
        raw_label_color = np.loadtxt('../data/toy/toy_problem_label_color.txt')
        raw_label_shape = np.loadtxt('../data/toy/toy_problem_label_shape.txt')
...
class Gaussian3DDataLoader(PrivacyDataLoader):
    def __init__(self, train_data=None, train_label=None, train_sensitive_label=None, test_data=None, test_label=None,
                 test_sensitive_label=None, trainset=None, testset=None):
        super().__init__(train_data, train_label, train_sensitive_label, test_data, test_label, test_sensitive_label, trainset, testset)
        self.name = "gaussian3D"

    def load(self):

        raw_train = torch.load('../data/gaussian3d/X_train')
        raw_test = torch.load('../data/gaussian3d/X_test')
        raw_train = torch.transpose(raw_train, 0, 1)
        raw_test = torch.transpose(raw_test, 0, 1)

        train_data = raw_train[:, 0:3]
        train_sensitive_label = raw_train[:, 4]
        train_label = raw_train[:, 3]
        test_data = raw_test[:, 0:3]
        test_sensitive_label = raw_test[:, 4]  # self.label[500:600]  # raw_label_color[500:600]
        test_label = raw_test[:, 3]
        ...
        self.train_label = train_label.long()
        self.train_sensitive_label = train_sensitive_label.long()

        self.trainset = MyDataset(self.train_data, self.train_label, self.train_sensitive_label)

        self.test_data = test_data.float()
        self.test_label = test_label.long()
        self.test_sensitive_label = test_sensitive_label.long()

        self.testset = MyDataset(self.test_data, self.test_label, self.test_sensitive_label)

        self.net = GaussianNet(input_dim=3, num_classes=self.n_target_class, embed_length=self.embed_length)
        self.target_net = SwissRollAdversary(num_classes=self.n_target_class, embed_length=self.embed_length)
        self.discriminator_net = SwissRollAdversary(num_classes=self.n_sensitive_class, embed_length=self.embed_length)
        self.adversary_net = SwissRollAdversary(num_classes=self.n_sensitive_class, embed_length=self.embed_length)

        self.optimizer = optim.Adam(self.net.parameters(), lr=0.0001, weight_decay=1e-3,
                                    betas=(0.9, 0.999))
        self.target_optimizer = optim.Adam(self.target_net.parameters(),
...
class ImageNetDataLoader(PrivacyDataLoader):
        def __init__(self, train_data=None, train_label=None, train_sensitive_label=None, test_data=None,
                     test_label=None,
                     test_sensitive_label=None, trainset=None, testset=None):
            self.name = "imagenet"
            super().__init__(train_data, train_label, train_sensitive_label, test_data, test_label,
                             test_sensitive_label, trainset, testset)
        ...
        def load(self):
            ...
            data_dir = '../../../research/hal-datastore/datasets/processed/ILSVRC/Data/256/'
            ...
            self.n_target_class = 2
            self.n_sensitive_class = 1000
            self.embed_length = 128  # 64
            self.train_batch_size = 128 # 128
            self.test_batch_size = 128 # 1000
            self.max_epoch_encoder = 200
            self.max_epoch_discriminator = 200

            self.trainloader = torch.utils.data.DataLoader(trainset, batch_size=self.train_batch_size, shuffle=True,
                                                           num_workers=40)
            self.testloader = torch.utils.data.DataLoader(testset, batch_size=self.test_batch_size, shuffle=False,
                                                          num_workers=40)

            self.train_size = len(trainset.imgs)  # self.train_data.shape[0]
            self.test_size = len(testset.imgs)

            self.nolife_set = np.concatenate((np.arange(398, 981), np.array([999])), axis=0)
            self.life_set = np.concatenate((np.arange(0, 398), np.arange(982, 999)), axis=0)
            ...
            self.model =  torchvision_resnet.resnet18(pretrained=True)#models.resnet18(pretrained=True)
            for param in self.model.parameters():
                param.requires_grad = False

            # num_ftrs = model_ft.fc.in_features
            # model_ft.fc = nn.Linear(num_ftrs, 256)
            # model_ft.fc1 = nn.Linear(256, 128)
            # model_ft.fc2 = nn.Linear(128, 64)
            # params_to_update = []
            ...
            self.net = create_discriminator(embed_length=512, type=3, num_classes=self.n_target_class)

            self.target_net = create_discriminator(embed_length=128, type=4,
                                                          num_classes=self.n_target_class)
            self.discriminator_net = create_discriminator(embed_length=128, type=4,
                                                          num_classes=self.n_sensitive_class)
            self.adversary_net = create_discriminator(embed_length=128, type=4,
                                                      num_classes=self.n_sensitive_class)
            self.optimizer = optim.Adam(self.net.parameters(), lr=0.0001, weight_decay=5e-2,
                                        betas=(0.9, 0.999))
            self.target_optimizer = optim.Adam(self.target_net.parameters(),
